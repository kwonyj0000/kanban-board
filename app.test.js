...
global.inviteMember          = jest.fn(async () => ({ error: null }));
global.getBoardMembers       = jest.fn(async () => []);
global.removeMember          = jest.fn(async () => ({ error: null }));
...
test('T-18: renderBoard — 주어진 데이터로 컬럼을 새로 렌더링한다', () => {
  addCardToColumn('todo', '기존 카드');
  renderBoard({ todo: ['새 카드'], inprogress: [], done: [] });
  const texts = document.querySelectorAll('#todo-list .card-text');
  expect(texts.length).toBe(1);
  expect(texts[0].textContent).toBe('새 카드');
});

test('T-19: renderBoard — 배지가 업데이트된다', () => {
  renderBoard({ todo: ['A', 'B'], inprogress: ['C'], done: [] });
  expect(document.getElementById('todo-badge').textContent).toBe('2');
  expect(document.getElementById('inprogress-badge').textContent).toBe('1');
  expect(document.getElementById('done-badge').textContent).toBe('0');
});

test('T-20: init — subscribeToBoardCards가 boardId와 함께 호출된다', async () => {
  window.__boardId = 'board-xyz';
  await init();
  expect(subscribeToBoardCards).toHaveBeenCalledWith('board-xyz', expect.any(Function));
});

test('T-21: renderMemberList — getBoardMembers 결과가 목록에 표시된다', async () => {
  document.body.innerHTML += '<ul id="member-list"></ul>';
  getBoardMembers.mockResolvedValueOnce([
    { id: 'm1', invited_email: 'friend@example.com', status: 'pending' },
    { id: 'm2', invited_email: 'other@example.com',  status: 'accepted' },
  ]);
  await renderMemberList();
  const items = document.querySelectorAll('.member-item');
  expect(items.length).toBe(2);
  expect(items[0].querySelector('.member-email').textContent).toBe('friend@example.com');
  expect(items[1].querySelector('.member-status').textContent).toBe('수락됨');
});

test('T-22: renderMemberList — 멤버 없을 때 안내 문구가 표시된다', async () => {
  document.body.innerHTML += '<ul id="member-list"></ul>';
  getBoardMembers.mockResolvedValueOnce([]);
  await renderMemberList();
  expect(document.querySelector('.member-empty')).not.toBeNull();
});

test('T-23: renderMemberList — 소유자일 때 삭제 버튼이 렌더링된다', async () => {
  document.body.innerHTML += '<ul id="member-list"></ul>';
  window.__isOwner = true;
  getBoardMembers.mockResolvedValueOnce([
    { id: 'm1', invited_email: 'a@b.com', status: 'pending' },
  ]);
  await renderMemberList();
  expect(document.querySelector('.remove-member-btn')).not.toBeNull();
});

test('T-24: renderMemberList — 멤버일 때 삭제 버튼이 없다', async () => {
  document.body.innerHTML += '<ul id="member-list"></ul>';
  window.__isOwner = false;
  getBoardMembers.mockResolvedValueOnce([
    { id: 'm1', invited_email: 'a@b.com', status: 'accepted' },
  ]);
  await renderMemberList();
